...
impl<'a> StateHandler<'a> {
    ...
    pub(crate) fn context_get(&self, id: NodeID) -> &NodeState {
        self.storage.get(&id).expect("Node component not found!")
    }

    pub(crate) fn context_get_mut(&mut self, id: NodeID) -> &mut NodeState {
        self.storage
            .get_mut(&id)
    ...
    pub(crate) fn context_insert(&mut self, id: NodeID) {
        ...
        self.storage.entry(id).insert_entry(NodeState::new(id));
    }
